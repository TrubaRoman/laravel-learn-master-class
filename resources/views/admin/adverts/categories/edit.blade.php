...
@section('content')
    ...
    <form  method="POST"  action="{{route('admin.adverts.categories.update',$category)}}">
        ...
        <div class="form-group">
            ...
            <input id="name" class="form-control{{$errors->has('name') ? ' is_invalid': ''}}" name="name" value="{{old('name',$category->name)}}" required>
            @if($errors->has('name'))
        ...
        <div class="form-group">
            ...
            <select id="parent" class="form-control{{$errors->has('parent') ? ' is_invalid': ''}}" name="parent" >
                <option value=""></option>
